...
class CanopyModel(FusionAlgorithm):
    ...
    def defineCharacteristics(self):
        ...
        self.addOutput(OutputFile(
            self.OUTPUT, self.tr('.dtm output surface'), 'dtm'))
        ...
        aspec = ParameterBoolean(
            self.ASPEC, self.tr('Calculate aspect'), False)
    ...
    def processAlgorithm(self, progress):
        ...
        aspec = self.getParameterValue(self.ASPEC)
        if aspec:
        ...
        if unicode(ret_num).strip():
            commands.append('/return:' + unicode(ret_num))
