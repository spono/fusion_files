...
class GridSurfaceCreate(FusionAlgorithm):
    ...
    def processAlgorithm(self, progress):
        ...
        if spike != 0.0:
            commands.append('/spike:' + unicode(self.getParameterValue(self.SPIKE)))
        median = self.getParameterValue(self.MEDIAN)
        if median != 0.0:
            commands.append('/median:' + unicode(self.getParameterValue(self.MEDIAN)))
        smooth = self.getParameterValue(self.SMOOTH)
        if smooth != 0.0:
            commands.append('/smooth:' + unicode(self.getParameterValue(self.SMOOTH)))
        slope = self.getParameterValue(self.SLOPE)
        if slope != 0.0:
            commands.append('/slope:' + unicode(self.getParameterValue(self.SLOPE)))
